...
def create_sequences(data, seq_length: int = 20, threshold: float = 0.01):
    """
    시퀀스 데이터와 레이블을 생성
    - data: 시계열 데이터
    ...
    for i in range(len(data) - seq_length):
        ...
        current_close = data[i + seq_length][0]
        previous_close = data[i + seq_length - 1][0]
...
def load_and_split_data(data_path, split_ratio: float = 0.8):
    """
...
def train_model(train_data, model_save_path, seq_length=20, learning_rate=0.001, epochs=50, hidden_size=64):
    """
    LSTM 모델을 학습하고 저장
    - train_data: 학습 데이터
